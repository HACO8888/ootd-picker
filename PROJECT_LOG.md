...
## 第 5 輪 loop（推薦演算法 v2 精修）

**範圍**：針對使用者回饋「推薦感覺怪怪的」調整 `src/lib/recommend.ts`，聚焦資料稀疏、整套搭配相容性、男性理容與換件品質。

### 產出

1. **語意相近補分**：保留核心權重（目的地 +5、天氣/季節 +4、心情 +3），新增目的地/心情相近標籤補分（例如約會⇄優雅、居家⇄舒適/放鬆、社交⇄優雅/活力），避免 catalog 缺少直接標籤時退成只看季節。
2. **天氣與性別脈絡補正**：冷天降低短袖/短褲/背心風險、雨天提高風衣/尼龍/功能包；男性情境提高襯衫/長褲/背包/電腦包，降低裙、內搭褲、手提包/手拿包、印度長衫等明顯不合候選。
3. **整套相容性排序**：新增 `totalOutfitScore`，在單品分數外評估上下身正式/休閒/運動/舒適輪廓、目的地配件、冷/雨外套需求與色彩和諧；約會/社交不再讓旅行袋、電腦包只靠顏色成為高分配件。
4. **A/B/C 近似去重**：候選池以商品名稱與服裝類型去重，避免 Fashion Product Images 多個同名商品讓結果看起來重複。
5. **換件品質修正**：`swapClosetItem` 改為帶入目前整套穿搭一起評分；妝容/香水「換一個」只從高分候選抽樣，修正原本可能從全資料集低分項目亂抽的問題。
6. **男性理容修正**：純男性情境只從男性理容候選中挑選，避免女性/中性妝容讓結果卡標題回到「推薦精緻妝容」。

### 驗收結果

| 項目 | 結果 | 證據 |
|---|---|---|
| 型別 | ✅ | `pnpm exec tsc --noEmit --pretty false` exit 0 |
| 推薦抽樣 | ✅ | 手動抽樣女性約會、男性約會、男女工作雨天、寒冷居家、社交聚會；冷/雨外套、男性理容、配件可留空、A/B/C 去重皆符合預期 |
